...
This guide adds drust to an existing Drupal 11 site. drust is an experiment:
try it on a staging copy first, and keep the rollback steps at the end handy.

You will:

1. Build the `drust.so` PHP extension for your server's PHP.
2. Install the `drust` Drupal module.
...
|---|---|---|
| Shared Rust cache store | The database cache backend (and APCu for 4 bins) | `$settings['cache']['default'] = 'cache.backend.drust_shared'` |
| Rust cache-tag checksums | The `cachetags` database table | `$settings['drust']['cache_tags'] = 'shared'` |
| CKEditor 5 plugin cache (PHP only) | Recomputing enabled CKEditor 5 plugins on every request | `$settings['drust']['ckeditor5_cache'] = TRUE` |
...
The result is `ext/dist/drust-php<version>-<arch>.so`. The build runs under
...
cd ext
cargo build --release
# The extension is target/release/libdrust.so
```
...
```sh
php -n -d extension=/path/to/drust.so -r 'echo drust_hello("server"), PHP_EOL;'
...
Copy `drupal/web/modules/custom/drust` from this repo into your site's
...
Put `drust.so` somewhere stable and readable by the web server user, for
example `/var/www/example.com/drust/drust.so`.
...
```ini
php_admin_value[extension] = /var/www/example.com/drust/drust.so
```
...
PHP process with an ini file, e.g. `/etc/php/8.5/mods-available/drust.ini`
containing `extension=/var/www/example.com/drust/drust.so`, enabled for both
`fpm` and `cli`.
...
exec php -d extension="$SITE/drust/drust.so" "$SITE/vendor/bin/drush.php" --root="$SITE" "$@"
```
...
- `shared_size_mb` must fit in the tmpfs. When the store reaches 70% it is
  emptied completely and refills; `/admin/reports/drust` shows how full it is.
- With `cache_tags` on, a process without the extension fails with
  `Call to undefined function drust_tags_get()`. That is deliberate: it is
...
# Drupal\drust\Cache\RustBackend
# Drupal\drust\Cache\RustCacheTagsChecksum
# Drupal\drust\CKEditor5\CachedCKEditor5PluginManager
```

Then:

- **Status page:** `/admin/reports/drust` shows the store's bins, items, size
  and hit rate. The site's status report (`/admin/reports/status`, or
  `bin/drush core:requirements`) has a "Drust PHP extension" entry: OK when
...
Running PHP workers have `drust.so` memory-mapped. **Never overwrite it in
place**, which can crash them. Upload the new file beside it, rename it over
the old one, then reload PHP:

```sh
cp drust-new.so /var/www/example.com/drust/drust.so.new
mv /var/www/example.com/drust/drust.so.new /var/www/example.com/drust/drust.so
sudo systemctl reload php8.5-fpm
sudo -u www-data bin/drush cr
```
...
| `You have requested a non-existent service "cache.backend.drust_shared"` | Settings added before the module was enabled | Remove the settings, enable the module, re-add them |
| `Call to undefined function drust_tags_get()` | A process without the extension (usually plain `vendor/bin/drush`) | Use `bin/drush`, or load the extension for that process |
